...
if [ "$REMEDIATE" = true ]; then
    ...
    #
    # Important: the classify branch used to skip pipeline enqueue via `elif`,
    # leaving `no_recent_pipeline_run` in FAILURES. Combined with `--notify`
    # that re-fired the Cursor webhook and agents re-dispatched remediates in
    # a loop (worker restart thrash). Always clear that failure after classify.
