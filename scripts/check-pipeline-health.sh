...
tier_a_restart_worker() {
    ...
    # Wait for Redis heartbeat before clearing the failure. Clearing blindly
    # made --remediate report "fixed" while the worker was still dead, which
    # re-fired webhooks and cascaded into restart thrashing.
    local i hb=""
...
if [ "$REMEDIATE" = true ]; then
    ...
    # queue jam also triggers classify (classify alone never starts ingest).
...
Warnings: $(IFS=, ; echo "${WARNINGS[*]}")"
    ...
    # Skip Cursor webhook after --remediate to avoid agent↔remediate feedback
    # loops. Telegram still notifies humans; non-remediate checks still webhook.
    if [ "$REMEDIATE" = true ]; then
