...
if [ "$REMEDIATE" = true ]; then
    ...
    # Classify drains a jammed queue / ready backlog. Missing ingest must still
    # enqueue the full cities pipeline — previously jam+no_recent_pipeline_run
    # took only the classify branch and left cron quiet (webhook thrash).
    ...
    if [ "$had_no_pipeline" = true ] || [ "$had_stale_ingest" = true ]; then
        if [ "$had_recent_ingest" = true ] && [ "$had_queue_jam" != true ]; then
            # Ingest already started; classify above is enough. Drop residual
            # failure so --notify cannot re-fire the Cursor webhook loop.
            filtered=()
            for f in "${FAILURES[@]}"; do
                [[ "$f" == no_recent_pipeline_run* ]] && continue
                [[ "$f" == backlog_active_but_no_recent_ingest* ]] && continue
            ...
            FAILURES=("${filtered[@]}")
            DETAILS+=("REMEDIATE: cleared_stale_ingest_failure_after_classify")
        else
