...
const MOCK_EVENTS: Map<number, PublicEvent> = new Map(
  MOCK_POINTS.map((p) => [
    p.id,
    {
      ...
      perpetrator_count: p.id % 3 === 0 ? 2 : 1,
      ...
      updated_at: p.d ?? new Date().toISOString(),
      sources: [
        {
          id: p.id * 10,
          ...
          google_news_url: 'https://news.google.com/rss/articles/mock',
          published_at: p.d,
          kind: 'source',
        },
        {
          id: p.id * 10 + 1,
          headline: `Cobertura complementar — ${p.c}`,
          publisher_name: 'Jornal local',
          url: null,
        ...
        {
          id: -(p.id * 10 + 2),
          headline: `Matéria sem link em ${p.n ?? p.c}`,
          publisher_name: null,
          url: null,
          google_news_url: null,
          published_at: p.d,
