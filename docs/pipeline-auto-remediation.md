...
| `arq_queue_jammed` | Clear ARQ locks + enqueue `classify_pending_task` (no restart). If `no_recent_pipeline_run` / `backlog_active_but_no_recent_ingest` also fire, also enqueue `ingest_cities_full_pipeline` |
