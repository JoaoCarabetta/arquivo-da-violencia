...
| Failure | Action |
|---------|--------|
| `worker_heartbeat_missing` | Restart worker, wait for heartbeat (fail if still missing), clear ARQ locks |
| `arq_queue_jammed` | Clear ARQ locks + enqueue `classify_pending_task` (no restart) |
| `no_recent_pipeline_run` / `backlog_active_but_no_recent_ingest` | Always enqueue `ingest_cities_full_pipeline` (even if queue jam also triggers classify) |
...
`--remediate --notify` still sends Telegram, but **skips the Cursor webhook** so agents do not re-dispatch remediates in a loop while the check is still unhealthy.
