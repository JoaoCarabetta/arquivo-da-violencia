...
In-flight city ingest (`ingest_city` / `process_city` / RSS fetch within the activity window) counts as recent ingest so long city sweeps do not false-positive `backlog_active_but_no_recent_ingest`.
