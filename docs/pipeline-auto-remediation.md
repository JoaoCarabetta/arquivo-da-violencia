...
**Notify loop guard:** `--remediate` skips the Cursor webhook even when `--notify` is set (Telegram still fires). Concurrent remediates take a flock. The GitHub workflow uses a single concurrency group and omits `--notify` for `repository_dispatch` remediate/diagnose.
