...
|----------|----------|---------|
| Describes **who** someone is (police, politician, occupation)? | Person models (see below) | Continue |
...
**Person inheritance** (do not duplicate shared fields on both roles):

```
IdentifiablePerson          ← shared: name, age, gender, occupation, description,
                              is_security_force, security_agent_type, security_agent_on_duty
├── IdentifiableVictim      ← + relationship_to_perpetrator, political_role
└── IdentifiablePerpetrator ← + relationship_to_victim

UnidentifiedPersonGroup     ← shared: count, description, is_security_force, is_civilian, context
├── UnidentifiedVictimGroup
└── UnidentifiedPerpetratorGroup
```

- **Shared person attribute** (applies to victims and perpetrators) → add once on `IdentifiablePerson`.
- **Victim-only** (e.g. `political_role`) → `IdentifiableVictim` only.
- **Perpetrator-only** (e.g. `relationship_to_victim`) → `IdentifiablePerpetrator` only.
- Keep JSON keys `victims` / `perpetrators` / `identifiable_victims` / `identifiable_perpetrators` stable — dedup and eval hard-code those paths.
- Do **not** collapse into `people: [{role: …}]`.
