...
## Person models (implemented)

Victims and perpetrators share a Pydantic base. Add shared attributes once; keep role wrappers and JSON keys stable.

```
IdentifiablePerson
├── IdentifiableVictim      (+ relationship_to_perpetrator, political_role)
└── IdentifiablePerpetrator (+ relationship_to_victim)

UnidentifiedPersonGroup
├── UnidentifiedVictimGroup
└── UnidentifiedPerpetratorGroup
```

| Field kind | Where to add |
|------------|--------------|
| Shared person attr (`occupation`, `security_agent_*`, …) | `IdentifiablePerson` |
| Victim-only (`political_role`, `relationship_to_perpetrator`) | `IdentifiableVictim` |
| Perpetrator-only (`relationship_to_victim`) | `IdentifiablePerpetrator` |
| Shared group attr | `UnidentifiedPersonGroup` |

Wrappers `Victims` / `Perpetrators` share `_validate_party_counts()` for list/count consistency. Persist `identified_perpetrator_count` on `unique_event` from `perpetrators.number_of_identifiable_perpetrators` (mirrors `identified_victim_count`).

**Do not** rename `victims` / `identifiable_victims` / `perpetrators` / `identifiable_perpetrators` — enrichment, dedup, and eval fixtures hard-code those paths.

---

## Schema: criminal group + politics + security agent (implemented)
...
### Security-agent fields (on `IdentifiablePerson`)

On `IdentifiableVictim` / `IdentifiablePerpetrator` (via `IdentifiablePerson`) when `is_security_force=true`:
...
Derived flat flag `security_force_victim` (victims only) powers map points `sv` and the synthetic **Policial vitimado** Por tipo count — not an `event_subtype`.

---

## Public surface — `derive_public_fields()`

Implement once in `backend/app/services/extraction_derived.py`:
...
| Column | Source |
|--------|--------|
| `security_force_involved` | any victim/perp `is_security_force` |
| `security_force_victim` | victims only `is_security_force` |
...
| `off_duty_police_context` | `homicide_dynamic.off_duty_police_context` |
...
| Model | When |
|-------|------|
| `IdentifiablePerson` | Base — shared person fields |
| `IdentifiableVictim` | Individual victim |
| `IdentifiablePerpetrator` | Individual author |
| `UnidentifiedPersonGroup` | Base — shared group fields |
| `UnidentifiedVictimGroup` | Victim group only |
| `UnidentifiedPerpetratorGroup` | Unnamed perpetrator group |

| Field kind | Identifiable* | Unidentified*Group |
|------------|---------------|---------------------|
| `political_role` | Victim only | No |
| `security_agent_type`, `security_agent_on_duty` | Yes (`IdentifiablePerson`) | No (unless whole group described) |
